...
import com.tyron.code.desktop.ui.control.richtext.linegraphics.RootLineGraphicFactory;
import com.tyron.code.desktop.ui.control.richtext.problem.ProblemTracking;
import com.tyron.code.desktop.ui.control.richtext.source.CompletionProvider;
import com.tyron.code.desktop.ui.control.richtext.syntax.StyleResult;
import com.tyron.code.desktop.ui.control.richtext.syntax.SyntaxHighlighter;
import com.tyron.code.desktop.ui.control.richtext.syntax.SyntaxUtil;
import com.tyron.code.desktop.util.FxThreadUtils;
import com.tyron.code.desktop.util.IntRange;
import com.tyron.code.project.util.ReflectUtil;
...
public class Editor extends BorderPane {
	...
	private final ExecutorService syntaxPool = Executors.newSingleThreadExecutor();
...
//	private ProblemOverlay problemOverlay;
	...
	private ContextMenu completionsMenu;
	private CompletableFuture<List<String>> previousCompletionRequest;
	...
	private void requestCompletion() {
		...
		previousCompletionRequest = CompletableFuture.supplyAsync(() -> {
			if (completionProvider == null) {
				return List.of();
			}
			return completionProvider.getCompletionSuggestions(this);
		});
		previousCompletionRequest.thenAccept(completions -> {
			if (completions.isEmpty()) {
				return;
			}
			FxThreadUtils.run(() -> {
				if (completionsMenu == null) {
					completionsMenu = new ContextMenu();
				}
				completionsMenu.hide();

				completionsMenu.getItems().clear();
				completions.stream().map(MenuItem::new).forEach(completionsMenu.getItems()::add);
				Bounds pointer = codeArea.caretBoundsProperty().getValue().orElseThrow();
				completionsMenu.show(this.getCenter(), pointer.getMinX(), pointer.getMinY() + pointer.getHeight());
			});
		});
	}
	...
	public String getText() {
		return Optional.of(codeArea.getText()).orElse("");
	}
...
//			problemOverlay.install(this);
	 ...
	 */
	public <T> CompletableFuture<Void> schedule(@NotNull ExecutorService supplierService,
												@NotNull Supplier<T> supplier, @NotNull Consumer<T> consumer) {
		return CompletableFuture.supplyAsync(supplier, supplierService)
				.thenAcceptAsync(consumer, FxThreadUtils::run);
	}
