...
import com.tyron.code.desktop.util.RegexUtil;
...
public class RegexSyntaxHighlighter implements SyntaxHighlighter {
	 ...
	 */
	public RegexSyntaxHighlighter(@NotNull RegexRule rootRule) {
		this.rootRule = rootRule;
	}

	@NotNull
	@Override
	public StyleSpans<Collection<String>> createStyleSpans(@NotNull String text, int start, int end) {
		try {
	...
	@NotNull
	@Override
	public IntRange expandRange(@NotNull String text, int initialStart, int initialEnd) {
	...
	public static class Region {
		...
		public List<String> currentClasses() {
			...
			return Stream.concat(parent.currentClasses().stream(), rule.classes().stream()).toList();
		}
		...
		public List<String> unmatchedClasses() {
			if (parent == null) return Collections.emptyList();
			return Stream.concat(parent.unmatchedClasses().stream(), parent.rule.classes().stream()).toList();
		}
	}
}
