...
public class FileManagerImpl implements FileManager {
        ...
        if (ignorePathPatterns.isEmpty()) {
            ...
            for (String pattern : ignorePathPatterns) {
                ...
                try {
                    ...
                    logger.warning("Invalid ignore path pattern " + pattern + ". \n" + t);
                }
            }
    ...
    public Optional<CharSequence> getFileContent(Path filePath) {
        ...
        try {
            return Optional.of(Files.readString(normalizedPath));
        } catch (Exception e) {
            logger.severe("Failed to read content from file " + normalizedPath + ".\n" + e);
        }
    ...
    public boolean isFileOpen(Path file) {
        return fileSnapshots.containsKey(file);
    }
